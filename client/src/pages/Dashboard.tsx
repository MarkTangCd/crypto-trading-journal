...
import { Loader2, TrendingUp, TrendingDown, Minus, Target, BarChart3, DollarSign, Percent, Hash, Flame, Wallet, Layers, CheckCircle2 } from "lucide-react";
import { useLocation } from "wouter";

export default function Dashboard() {
  const [, setLocation] = useLocation();
  const { data: stats, isLoading: statsLoading } = trpc.stats.get.useQuery();
  const { data: systemStats, isLoading: systemStatsLoading } = trpc.stats.getBySystem.useQuery();
  ...
  const statCards = [
    ...
    },
    {
      ...
      color: stats.latestBalance >= stats.originalBalance ? "status-win" : "status-loss",
    },
  ];

  const balanceChange = stats.latestBalance - stats.originalBalance;
  const balanceChangePercent = stats.originalBalance > 0 
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-subtitle mt-1">Overview of your trading performance</p>
      </div>

      {/* Summary Cards */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10" style={{ background: "var(--color-pastel-blue)", transform: "translate(30%, -30%)" }} />
          <CardHeader className="pb-2">
            <CardDescription className="text-subtitle">Account Performance</CardDescription>
            <CardTitle className="text-3xl font-bold">
              <span className={balanceChange >= 0 ? "status-win" : "status-loss"}>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              {balanceChange >= 0 ? "+" : ""}${balanceChange.toFixed(2)} from original
            </p>
          </CardContent>
        </Card>

        <Card className="relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10" style={{ background: "var(--color-pastel-pink)", transform: "translate(30%, -30%)" }} />
          <CardHeader className="pb-2">
            <CardDescription className="text-subtitle">Win Rate</CardDescription>
            <CardTitle className="text-3xl font-bold">
              <span className={stats.winRate >= 50 ? "status-win" : "status-loss"}>
        ...
        <Card className="relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10" style={{ background: "var(--color-pastel-blue)", transform: "translate(30%, -30%)" }} />
          <CardHeader className="pb-2">
            <CardDescription className="text-subtitle">Net Reward</CardDescription>
            <CardTitle className="text-3xl font-bold">
              <span className={stats.totalReward >= 0 ? "status-win" : "status-loss"}>
        ...
        <Card className="relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10" style={{ background: "var(--color-pastel-pink)", transform: "translate(30%, -30%)" }} />
          <CardHeader className="pb-2">
            <CardDescription className="text-subtitle">Current Balance</CardDescription>
            <CardTitle className="text-3xl font-bold">${stats.latestBalance.toFixed(2)}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-subtitle">Win rate statistics by trading system</CardDescription>
              </div>
              <Button variant="outline" size="sm" onClick={() => setLocation("/trading-systems")}>
                Manage Systems
              </Button>
            </div>
          </CardHeader>
              ...
              {sortedSystemStats.map((system) => (
                <div
                  ...
                  className={`p-4 rounded-lg border transition-all ${
                    system.isActive ? "border-green-200 bg-green-50/30" : "bg-muted/30"
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className={`font-semibold ${system.winRate >= 50 ? "status-win" : "status-loss"}`}>
                        ...
                        <div>
                          ...
                          <p className="font-semibold">
                            <span className="status-win">{system.winCount}</span>
                            {" / "}
                            <span className="status-loss">{system.lossCount}</span>
                            {" / "}
                            <span className="status-breakeven">{system.breakevenCount}</span>
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Total Return</p>
                          <p className={`font-semibold ${system.totalReturn >= 0 ? "status-win" : "status-loss"}`}>
                    ...
                    <div className="hidden sm:block w-24 shrink-0">
                      <div className="text-center mb-1">
                        <span className={`text-lg font-bold ${system.winRate >= 50 ? "status-win" : "status-loss"}`}>
      ...
      {sortedSystemStats.length === 0 && stats.totalTrades > 0 && (
        <Card className="border-dashed">
          <CardContent className="py-8 text-center">
            ...
            <h3 className="font-semibold mb-1">Track Performance by Trading System</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Create trading systems to categorize your trades and analyze which strategies work best
            </p>
      ...
      {stats.totalTrades > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-subtitle">Key observations from your trading data</CardDescription>
          </CardHeader>
          <CardContent>
