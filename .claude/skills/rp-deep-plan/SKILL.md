...
**Capture the findings — don't just absorb them.** The explore agents did real reconnaissance, but they also return a lot. Curate: distill the _load-bearing_ evidence — file:line refs, type names, extension points, links, prior art (including anything useful the Phase 1.5 ambiguity scouts surfaced) — into the plan's `## Background` when you scaffold the file next. The goal is enough grounding that `context_builder` doesn't re-derive seams from scratch — not a verbatim dump of every agent's output. When unsure whether a concrete reference matters, keep it; leave the raw transcripts and narration behind.
...
The tool returns `oracle_export_path`. **The export is your draft** — `context_builder`'s plan pass is more grounded than your scaffold, and if it framed the approach and work items well, that framing _is_ design output worth keeping. Build the plan body _out of it_; don't pre-edit it down.
...
{
  "tool": "file_actions",
  "args": { "action": "delete", "path": "<oracle_export_path>" }
}
```
...
- 🚫 Treating the `context_builder` export as a skeleton to mine — it's your _draft_; build the plan out of it and keep its framing where it's good
